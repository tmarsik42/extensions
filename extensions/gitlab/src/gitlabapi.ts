...
export class GitLab {
    ...
    public async fetch(url: string, params: { [key: string]: string } = {}, all: boolean = false): Promise<any> {
        const per_page = all ? 100 : 50;
        let fetchPage = async (page: number): Promise<Response> => {
    ...
    async getProjectMember(projectId: Number): Promise<User[]> {
        const userItems: User[] = await this.fetch(`projects/${projectId}/users`)
            .then((users) => {
    ...
    async getProjectMilestones(projectId: Number): Promise<Milestone[]> {
        const items: Milestone[] = await this.fetch(`projects/${projectId}/milestones`)
            .then((labels) => {
                return labels.map((data: any, index: number) => ({
                    id: data.id,
                    title: data.title,
    ...
    async getGroupMilestones(group: Group): Promise<Milestone[]> {
        const items: Milestone[] = await this.fetch(`groups/${group.id}/milestones`)
            .then((labels) => {
                return labels.map((data: any, index: number) => ({
                    id: data.id,
                    title: data.title,
    ...
    async getUserProjects(params: Record<string, any> = {}, all: boolean): Promise<Project[]> {
        ...
        return await this.fetch("projects", params = params, all)
            .then((projects: any[]) => {
    ...
    async getProjects(args = { searchText: "", searchIn: "" }): Promise<Project[]> {
        ...
        const issueItems: Project[] = await this.fetch("projects", params = params)
            .then((projects) => {
    ...
    async getProject(projectID: number): Promise<Project> {
        const pro: Project = await this.fetch(`projects/${projectID}`)
            .then((project) => {
    ...
    async getStarredProjects(args = { searchText: "", searchIn: "" }, all: boolean): Promise<Project[]> {
        ...
        const projects: Project[] = await this.fetch(`users/${user.id}/starred_projects`, params = params, all)
            .then((projects: any[]) => {
                return projects.map((p: any) => dataToProject(p));
            });
        return projects;
    ...
    async getUsers(args = { searchText: "", searchIn: "" }): Promise<User[]> {
        ...
        const userItems: User[] = await this.fetch("users", params = params)
            .then((users) => {
                return users.map((userdata: any, index: number) => ({
                    id: userdata.id,
                    name: userdata.name,
    ...
    async getTodos(params: Record<string, any>): Promise<Todo[]> {
        const issueItems: Todo[] = await this.fetch("todos", params = params)
            .then((issues) => {
    ...
    async getMyself(): Promise<User> {
        const user: User = await receiveLargeCachedObject("user", async () => {
            const user: User = await this.fetch("user")
                .then((userdata) => {
                    return {
                        id: userdata.id,
                        name: userdata.name,
    ...
    async getUserGroups(params: Record<string, any> = {}): Promise<any> {
        ...
        const dataAll: Group[] = await receiveLargeCachedObject(hashRecord(params, "mygroups"), async () => { return ((await this.fetch(`groups`, params, true)) as Group[]) || []; });;
...
export async function searchData<Type>(data: any, params: { search: string, keys: string[], limit: number, threshold?: number, ignoreLocation?: boolean }) {
    const options = {
        includeScore: true,
